...
def remove_outliers(pred_GndSeg, GndSeg): # removes the points outside grid and unlabled points
    ...
    index = np.logical_and(pred_GndSeg >= 0, GndSeg >=0)
    pred_GndSeg = pred_GndSeg[index]
    GndSeg = GndSeg[index]

    return 1-pred_GndSeg, 1-GndSeg, index
...
def evaluate_SemanticKITTI(data_dir):
    ...
    frame_cnt = 0
    print(len(frames))
    for f in range(len(frames)): # len(frames)
        ...
        pred_GndSeg, GndSeg, index = remove_outliers(pred_GndSeg, GndSeg)
        points_pred_labeled = np.copy(points)
        points_pred_labeled[:, 3] = -1
        points_pred_labeled[index, 3] = pred_GndSeg
        np.save('kitti-pred.npy', points_pred_labeled)
        points_truth_labeled = np.copy(points)
        points_truth_labeled[:, 3] = -1
        points_truth_labeled[index, 3] = GndSeg
        # print(points_truth_labeled[index, 3].shape)
        np.save('kitti-ground-truth-label.npy', points_truth_labeled)
        pred_grids = calculate_grid_label(1, points_pred_labeled)
        truth_grids = calculate_grid_label(1, points_truth_labeled)
        precision = calculate_precision(pred_grids, truth_grids)
        ...
        print(f, iou, mse, prec, recall) # , precision

        frame_cnt += 1
        print(grid_precision/frame_cnt)
    ...
    print('Grid precision: ', grid_precision/len(frames) )
