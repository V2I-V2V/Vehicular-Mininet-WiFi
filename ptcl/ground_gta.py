...
if __name__ == '__main__':
    ...
    pcl = np.fromfile(ptcl_path[0]+'000000.bin', dtype=np.float32, count=-1).reshape([-1,4])
    print(pcl.shape)
    t1 = time.time()
    p2, p1, best_model, _ = my_ransac_v5(pcl, max_trials, P=0.8, distance_threshold=0.10, lidar_height=-1.57727+0.05, lidar_height_down=-1.57727-0.15)  # pcl, P=0.8, distance_threshold=0.15, lidar_height=-h+0.05 0.15, lidar_height_down=-h-0.15 -0.2
    t2 = time.time()
    print(t2-t1)
    pcl1 = pcl[p1]  # object
    pcl2 = pcl[p2]  # ground
    ...
    pcd_object.points = o3d.utility.Vector3dVector(pcl2[:,:3])
